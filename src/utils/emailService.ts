...
const createExcelFile = (inventoryItems: InventoryItem[]): File => {
  ...
  // Renk bilgileri
  const primaryColor = '4A6BFF'; // Mavi
  const accentColor = '2ECC71';  // Yeşil
  const borderColor = 'E1E8F5';  // Açık Gri
  
  // Başlık stili
  const headerStyle = {
    font: { bold: true, color: { rgb: 'FFFFFF' } },
    fill: { patternType: 'solid', fgColor: { rgb: primaryColor } },
  ...
  // Adet stili
  const countStyle = {
    font: { bold: true, color: { rgb: accentColor } },
  ...
  worksheet['A1'] = { v: 'Ürün', t: 's', s: headerStyle };
  worksheet['B1'] = { v: 'Barkod', t: 's', s: headerStyle };
  worksheet['C1'] = { v: 'Adet', t: 's', s: headerStyle };
  
  // Veri hücrelerini stil ile biçimlendir
  const range = XLSX.utils.decode_range(worksheet['!ref'] || 'A1:C1');
  for (let row = range.s.r + 1; row <= range.e.r; row++) {
    // Ürün ve barkod hücreleri için normal stil
    worksheet[`A${row+1}`] = { 
      ...worksheet[`A${row+1}`], 
      s: normalStyle 
    };
    ...
    // Adet hücresi için özel stil
    if (worksheet[`C${row+1}`]) {
      worksheet[`C${row+1}`].s = countStyle;
    }
  }
  
  // Stok sayım özet bilgileri
  ...
  const summaryData = [
    ...
    ['Toplam Farklı Ürün:', totalItems],
    ['Toplam Sayılan Adet:', totalCount],
    [''],
    ['Bu rapor stok sayım uygulaması tarafından oluşturulmuştur.']
  ];
  
  const summaryWs = XLSX.utils.aoa_to_sheet(summaryData);
  
  // Özet sayfa başlığı için stil
  summaryWs['A1'] = { 
    v: 'STOK SAYIM RAPORU', 
    t: 's',
    s: { 
      font: { bold: true, sz: 16, color: { rgb: primaryColor } },
      alignment: { horizontal: 'center' }
    }
  };
  
  // Özet için kolon genişliği
  summaryWs['!cols'] = [{ wch: 20 }, { wch: 20 }];
  
  // Başlık hücresini birleştir
  summaryWs['!merges'] = [
    ...
    <table style="width:100%; border-collapse: collapse; margin-bottom: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
      <thead>
        <tr style="background: #4A6BFF;">
          <th style="text-align: left; padding: 12px 10px; font-size: 14px; font-weight: 600; color: white; border: 1px solid #E1E8F5;">Ürün</th>
  ...
  for (let i = 0; i < maxItems; i++) {
    ...
    itemsHtml += `
      <tr style="background-color: ${rowColor};">
        <td style="text-align: left; padding: 10px; font-size: 14px; font-weight: 400; border: 1px solid #E1E8F5;">${item.name}</td>
        <td style="text-align: center; padding: 10px; font-size: 14px; color: #4A6BFF; border: 1px solid #E1E8F5;">${item.barcode}</td>
      ...
      <div style="max-width: 650px; margin: 0 auto; background-color: #ffffff; box-shadow: 0 3px 10px rgba(0,0,0,0.1);">
        ...
        <div style="padding: 40px 30px; background-color: #ffffff;">
          ...
          <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #eee;">
            <p style="margin-bottom: 5px;">Saygılarımızla,</p>
        ...
        <div style="background-color: #f8faff; padding: 20px; text-align: center; border-top: 1px solid #eee; font-size: 13px; color: #7f8c8d; border-radius: 0 0 6px 6px;">
          <p style="margin: 0;">Bu email otomatik olarak <span style="color: #4a6bff; font-weight: 600;">Stok Sayım Uygulaması</span> tarafından gönderilmiştir.</p>
